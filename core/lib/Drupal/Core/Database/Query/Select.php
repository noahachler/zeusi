...
class Select extends Query implements SelectInterface {
  ...
  public function addJoin($type, $table, $alias = NULL, $condition = NULL, $arguments = []) {
    ...
    $count = 2;
    // FIX ZEUSI
    while (!empty($this->tables[$alias_candidate]) || $this->arrayContainValue($alias_candidate, $this->tables)) {
  ...
  // FIX ZEUSI
  private function arrayContainValue($value, $array) {

    foreach ($array as $item) {
      if (array_search($value, $item) === 'table') {
        return true;
      }
    }
